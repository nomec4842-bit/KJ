 ...
 * @returns {{gridCells:HTMLElement[], update:()=>void, paint:(i:number)=>void}}
 */
export function createGrid(seqEl, onToggle, onSetVel) {
  ...
  for (let i = 0; i < NUM_STEPS; i++) {
    ...
    // Click: toggle/cycle velocity (the toggle handler should handle cycling)
    cell.addEventListener('click', () => onToggle(i));

    // Drag: set velocity continuously
  ...
  // Render helpers (main passes a getter for current steps when calling update)
  function update(getStep) {
    for (let i = 0; i < NUM_STEPS; i++) {
      ...
      if (bar) bar.style.height = st?.on ? Math.round((st.vel || 1) * 100) + '%' : '0';
    }
  }
  function paint(i) {
