...
function normalizeTrack(t) {
  ...
  if (!t.cvl || typeof t.cvl !== 'object') {
    t.cvl = { lanes: 6, samples: [], clips: [], scrubberRate: '1/16', scrubberDepth: 0, snap: true };
  } else {
...
async function onCvlSampleFile(file) {
  ...
  track.cvl.samples.push({ name: file.name, duration: buffer.duration });
  sampleCache[file.name] = buffer;
  renderCvlPanel();
  saveProjectToStorage();
}
...
function renderCvlPanel() {
  ...
  const snapToggle = document.getElementById('cvl_snap');
  if (snapToggle) {
  ...
  const binItems = cvlRoot.querySelectorAll('.cvl-bin li[draggable="true"]');
  ...
  const laneTracks = cvlRoot.querySelectorAll('.cvl-lane-track');
  ...
  const clipEls = cvlRoot.querySelectorAll('.cvl-clip');
